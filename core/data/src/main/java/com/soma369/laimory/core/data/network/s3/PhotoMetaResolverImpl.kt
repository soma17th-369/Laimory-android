                ...
                try {
                    val contentType =
                        resolver.getType(uri)
                            ?: throw DraftPhotoAccessException("사진 MIME 타입을 확인할 수 없습니다: $clientPhotoUri")
                    val size =
                        resolver.openAssetFileDescriptor(uri, "r")?.use { it.length }
                            ?.takeIf { it >= 0L }
                            ?: throw DraftPhotoAccessException("사진 크기를 확인할 수 없습니다: $clientPhotoUri")
                    ...
                    // FileNotFoundException·SecurityException 등 URI open/권한 실패도 사진 접근 오류로 정규화한다.
                    throw DraftPhotoAccessException("사진에 접근할 수 없습니다: $clientPhotoUri")
                }
            }
    }
