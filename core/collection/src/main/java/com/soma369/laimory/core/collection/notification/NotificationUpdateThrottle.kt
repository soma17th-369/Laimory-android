 ...
 * 억제는 두 신호가 모두 없는 갱신형 알림에 대한 안전망이다. 정상 앱의 상태 전이 알림(주문 접수 →
...
internal class NotificationUpdateThrottle(
     ...
     * 이후 [MINIMUM_INTERVAL] 이 억제되어 한 건을 잃지만, 저장 완료를 기다렸다가 기록하면 그
     * 사이 도착한 갱신이 억제를 빠져나가 초 단위 갱신에 더 약해진다.
     */
    @Synchronized
    fun markCollected(
        key: String,
    ...
    companion object {
         ...
         *
         * 잔여량이 남는다는 것을 알고 쓴다 — 최소 간격은 "얼마나 자주" 만 보고 "총 몇 건" 은 보지
         * 않아서, 갱신이 오래 이어질수록 수집량이 선형으로 는다. 초당 갱신이 2시간 27분 이어진
         * 실기기 사례에 대입하면 6,126건이 약 147건으로 줄 뿐이고, 같은 기기의 정상 알림 전체가
         * 26건이었으므로 이 잔여량만으로도 초안 상한을 잠식한다. 상주 알림·읽을 수 없는 본문
         * 제외가 1차 방어인 이유다.
         *
         * 잔여량을 상수로 묶으려면 같은 키의 일일 수집 상한이 맞지만, 서비스 재시작을 견디려면
         * 영속 저장이 따라와야 해서 이 클래스의 범위를 넘는다.
         */
