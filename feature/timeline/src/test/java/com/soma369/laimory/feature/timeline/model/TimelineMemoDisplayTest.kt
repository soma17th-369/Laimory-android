...
class TimelineMemoDisplayTest {
    ...
    fun `메모와 질문이 모두 없으면 편집 모드에서만 기본 문구를 보여준다`() {
        val editable = timelineMemoDisplay(memo = null, question = null, isEditable = true)
        val readOnly = timelineMemoDisplay(memo = null, question = null, isEditable = false)

        assertEquals(TimelineMemoDisplay.Prompt(DEFAULT_MEMO_PROMPT), editable)
        // 읽기 모드에 누를 수 없는 빈 입력칸을 남기지 않는다.
        assertNull(readOnly)
    }

    @Test
    fun `읽기 모드에서도 질문은 보여준다`() {
        val display = timelineMemoDisplay(memo = null, question = "오늘 어땠나요?", isEditable = false)

        assertEquals(TimelineMemoDisplay.Question("오늘 어땠나요?"), display)
    }
