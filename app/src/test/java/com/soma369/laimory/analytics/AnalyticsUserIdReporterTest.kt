...
class AnalyticsUserIdReporterTest {
    private val account = SignedInAccount(provider = SocialLoginProvider.GOOGLE)

    @Test
    fun `회원 식별자를 받으면 그 회원으로 건다`() {
        assertEquals(
            AnalyticsUserIdChange.Assign(42L),
            analyticsUserIdChange(account, UserProfile.of("김소마", userId = 42L)),
        )
    }

    @Test
    fun `세션이 없으면 푼다`() {
        assertEquals(AnalyticsUserIdChange.Clear, analyticsUserIdChange(account = null, profile = null))
    }
    ...
    fun `로그아웃 직후 회원 정보가 남아 있어도 푼다`() {
        ...
        assertEquals(
            AnalyticsUserIdChange.Clear,
            analyticsUserIdChange(account = null, profile = UserProfile.of("김소마", userId = 42L)),
        )
    }
    ...
    fun `세션은 있는데 회원 정보를 아직 못 받았으면 그대로 둔다`() {
        ...
        assertNull(analyticsUserIdChange(account, profile = null))
    }

    @Test
    fun `회원 식별자를 주지 않는 서버에서는 그대로 둔다`() {
        assertNull(analyticsUserIdChange(account, UserProfile.of("김소마", userId = null)))
    }
}
